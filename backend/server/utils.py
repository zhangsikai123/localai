...
from configs import EMBEDDING_DEVICE
from configs import FSCHAT_MODEL_WORKERS
from configs import HTTPX_DEFAULT_TIMEOUT
from configs import LANGCHAIN_LLM_MODEL
from configs import LLM_DEVICE
...
def outside_api_address() -> str:
    from configs.server_config import OUTSIDE_API_SERVER

    host = OUTSIDE_API_SERVER["host"]
    if host == "0.0.0.0":
        host = "127.0.0.1"
    port = OUTSIDE_API_SERVER["port"]
    return f"http://{host}:{port}"
